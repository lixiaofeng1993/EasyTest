...
from lib.public import token_sign, check_token
...
def login(request):
    if request.method == "POST":
        ...
        if password_check:
            ...
            token = token_sign(username)
            return JsonResponse({"status": 200, "message": "login successfully!", "token": token})
        else:
